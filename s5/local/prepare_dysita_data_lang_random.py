...
Furthermore it could be pass as parameter a other flag:
 ...
 --users-list: users id list of speakers that we want analyse
 --random: random wave file selection for test and train set
The two flags could be use at the same time, it means that the software will find the users who have that ID and that
...
parser.add_argument(
    ...
    type=str,
    default='ALL',
    help="""Specify the ID users list to analyse. To use all users type ALL.
		 You can pass more then one user in this way: --users-list 201,304
		 If you use this flag combined with --disease, it take into account 
		 just users with that specific disease.""",
)
...
parser.add_argument(
    ...
    type=boolean,
    default='False',
    help="""Specify if the generation of Train and Test set is made randomly""",
)




spkIdList = sys.argv[2:]

listOfSpeaker = {}
if os.path.isdir(dbDir):
	if spkIdList[0] == 'all':
		for spIdCode in os.listdir(dbDir):
			parseSpeaker(listOfSpeaker, spIdCode, dbDir)
	else:
		for spIdCode in spkIdList:
			parseSpeaker(listOfSpeaker, spIdCode, dbDir)
	print('Tot number of users: '+str(len(listOfSpeaker)))
else:
	print("Error! Database folder not correct: "+dbDir)
	exit()
# insert the speaker "ALL"
"""spALL = Speaker(-1)
for sp in spkIdList:
	spALL.mergeSpeakers(listOfSpeaker[sp])
listOfSpeaker['ALL'] = spALL"""
...
for sp in listOfSpeaker.keys():
	spALL.mergeSpeakers(listOfSpeaker[sp])
generateKaldiFile(spALL.getTestAndTrainSet(), './', wordsList, list(phonemes.listOfPhonemes.keys()))
